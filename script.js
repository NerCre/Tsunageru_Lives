...
(() => {
  ...
  function concludeEmergency() {
    showOverlay(
      '<div>救護活動を継続してください。<br>AED到着後、すぐに使用してください。</div><div class="overlay-illustration"><img src="AED.png" alt="AEDの使用手順" /></div>',
      [
...
function renderCompanyList() {
  ...
  function wireGlobalEvents() {
    ...
    // Back to overview button on AED map view
    const aedOverviewBtn = $('#btnAedOverview');
    if (aedOverviewBtn)
      aedOverviewBtn.addEventListener('click', () => {
        const ov = document.getElementById('aed-overview');
        const detail = document.getElementById('aed-detail');
        if (ov) ov.classList.remove('hidden');
        if (detail) detail.classList.add('hidden');
        const img = document.getElementById('aedDetailImg');
        if (img) img.src = '';
      });
    ...
    // Back to overview button on stretcher map view
    const stretcherOverviewBtn = $('#btnStretcherOverview');
    if (stretcherOverviewBtn)
      stretcherOverviewBtn.addEventListener('click', () => {
        const ov = document.getElementById('stretcher-overview');
        const detail = document.getElementById('stretcher-detail');
        if (ov) ov.classList.remove('hidden');
        if (detail) detail.classList.add('hidden');
        const img = document.getElementById('stretcherDetailImg');
        if (img) img.src = '';
      });
    ...
    // Back to overview button on OS-1 map view
    const os1OverviewBtn = $('#btnOs1Overview');
    if (os1OverviewBtn)
      os1OverviewBtn.addEventListener('click', () => {
        const ov = document.getElementById('os1-overview');
        const detail = document.getElementById('os1-detail');
        if (ov) ov.classList.remove('hidden');
        if (detail) detail.classList.add('hidden');
        const img = document.getElementById('os1DetailImg');
        if (img) img.src = '';
      });
