      ...
      [
        {
          ...
          style: 'primary',
          onClick: () => {
            // オーバーレイを閉じてAEDマップを表示し、マップを閉じたら再度物品選択に戻る
            closeOverlay();
            if (typeof showAedMap === 'function') {
              showAedMap(() => {
                // 再び物品選択を表示する
  ...
  function initOnePage() {
    ...
    if (btnSms) {
      btnSms.addEventListener('click', () => {
        ...
        const statusStr = `意識${conscious}、呼吸${breathing}、大量出血${bleeding}、強い痛み${pain}`;
        // 事故種別
        let accStr = '不明';
        ...
        lines.push(`送信時間：${sendTime}`);
        lines.push(`職員ID：${staffId}`);
        lines.push(`場所：${locName}`);
        lines.push(`状態：${statusStr}`);
